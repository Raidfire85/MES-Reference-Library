...
export function resolveSyncSectionCopy(context: SyncSectionContext): SyncSectionCopy {
  if (context.htmlFile && PAGE_SUPPLEMENT_COPY[context.htmlFile]) {
    return PAGE_SUPPLEMENT_COPY[context.htmlFile];
  }
...
export function pageTitleFromHtmlFile(htmlFile: string): string {
  let stem = htmlFile.replace(/\.html$/i, '');
  if (stem.endsWith('-Profile')) {
    stem = stem.slice(0, -'-Profile'.length);
  }

  const known: Record<string, string> = {
    'Spawning-Conditions': 'Spawn Condition',
